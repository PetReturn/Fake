...
class PortalManagerView(ModalView):
    def __init__(self, main_screen, **kwargs):
        ...
        container.add_widget(Label(text="[color=00E6FF][b]MAC ULTRA FAVORITEN[/b][/color]", markup=True, size_hint_y=None, height=60, font_size="22sp"))
        
        input_card = StyledCard(orientation="vertical", size_hint_y=None, height=180, padding=10, spacing=10)
        self.new_portal_input = TextInput(hint_text="http://url.com:8080", multiline=False, height=50, size_hint_y=None, background_color=(0.1, 0.1, 0.1, 1), foreground_color=(1,1,1,1))
    ...
    def create_entry(self, url, p_type):
        btn = StyledButton(text=url[:30], font_size="12sp", size_hint_y=None, height=50, on_press=lambda x: self.select_portal(url))
        return btn
...
class MagUltraScreen(Screen):
    def __init__(self, context, **kw):
        ...
        self.detail_status = Label(text="NUR URL", color=RED, size_hint_x=0.4)
        ...
        self.delay_slider = Slider(min=0.0, max=2.0, value=0.1, step=0.05, size_hint_x=0.8)
        self.delay_slider.bind(value=lambda i, v: setattr(self.delay_value_display, 'text', f"{v:.2f}s"))
        delay_row.add_widget(self.delay_value_display); delay_row.add_widget(self.delay_slider)
        ...
        self.scroll = ScrollView(); self.log_display = Label(text="Bereit...", size_hint_y=None, markup=True, halign="left"); self.log_display.bind(size=self.log_display.setter('text_size')); self.scroll.add_widget(self.log_display); self.box.add_widget(self.scroll)
    ...
    def open_favs(self, *a):
        PortalManagerView(main_screen=self).open()

    def update_slider_label(self, i, v):
        self.detail_status.text, self.detail_status.color = ("ALLES SPEICHERN", GREEN) if v == 1 else ("NUR URL", RED)
    ...
    async def engine_dummy(self):
        ...
        while self.running and self.checked < self.total_lines:
            ...
            self.checked += 1
            if random.random() > 0.9: self.hits += 1; self.update_log_safe(f"[color=00FF80]HIT:[/color] Found at {self.checked}")
            Clock.schedule_once(lambda dt: self.refresh_ui())
        self.running = False
        Clock.schedule_once(self.reset_start_button)
...
def create_app_screen(context):
    """Gibt den Hauptscreen zurück. Der Favoriten-Manager ist als ModalView integriert."""
